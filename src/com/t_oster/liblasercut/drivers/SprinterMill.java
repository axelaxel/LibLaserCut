...
/**
 * This class implements a driver for a mill with Sprinter 3D printer firmware.
 * Currently it supports the simple code and the G-Code, which may be used in
 * the future.
 *
 * @author Thomas Oster <thomas.oster@rwth-aachen.de>
 */
public class SprinterMill extends LaserCutter
{
  ...
  @Override
  public SprinterMillProperty getLaserPropertyForVectorPart()
  {
    return new SprinterMillProperty();
  }

  @Override
  public SprinterMillProperty getLaserPropertyForRasterPart()
  {
    return new SprinterMillProperty();
  }

  @Override
  public SprinterMillProperty getLaserPropertyForRaster3dPart()
  {
    return new SprinterMillProperty();
  }
  ...
  {
    if (p instanceof SprinterMillProperty)
    {
      SprinterMillProperty prop = (SprinterMillProperty) p;
    ...
    {
      throw new RuntimeException("The SprinterMill driver only accepts SprinterMill properties (was "+p.getClass().toString()+")");
    }
  }
  ...
  private byte[] generateSprinterMillRasterCode(RasterPart rp, double resolution) throws UnsupportedEncodingException, IOException
  {
    ByteArrayOutputStream result = new ByteArrayOutputStream();
    PrintStream out = new PrintStream(result, true, "US-ASCII");
    boolean dirRight = true;
  ...
  {
    SprinterMill clone = new SprinterMill();
